...
extension AddressFormViewController {
    ...
    private func checkAddressRequestCompletion(result: AtlasResult<CheckAddressResponse>) {
        loaderView.hide()
        guard let checkAddressResponse = result.handleError(checkoutProviderType: self) else { return }
    ...
    private func createUpdateAddressRequestCompletion(result: AtlasResult<UserAddress>) {
        loaderView.hide()
        guard let address = result.handleError(checkoutProviderType: self) else { return }
        dismissView()
        completion?(address)
    }
